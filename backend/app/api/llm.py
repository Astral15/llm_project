...
if not s.GEMINI_API_KEY:
  # hard fail: this service makes no sense without an API key
...
class LLMStructuredResponse(BaseModel):
  data: Dict[str, Any]
  from_cache: bool = False  # set true when hit DB cache
...
def _build_schema(fields: List[FieldSpec]) -> dict:
  # basic OBJECT schema: each field is STRING or NUMBER, all required
  props: Dict[str, Any] = {}
  req: List[str] = []
  for f in fields:
...
def _cache_key(
  ...
  # deterministic key over logical input, shared across all users
  payload = {
    "p": prompt,
...
def _get_cached(db: Session, ck: str) -> Optional[Dict[str, Any]]:
  # use latest response for this cache key (id desc)
  req = (
    db.query(LLMRequest)
    .filter(LLMRequest.cache_key == ck)
  ...
  resp = (
    ...
    .filter(LLMResponse.request_id == req.id)
...
def _store_cache(
  ...
  # raw_response and validated_response are the same for now; hook for a validator
  req = LLMRequest(
    user_id=user_id,
    prompt=body.prompt,
  ...
  db.add(req)
  db.flush()  # assign req.id without full commit
...
def get_structured_response(
  ...
  # cache key spans prompt, field structure, and stable image content hash
